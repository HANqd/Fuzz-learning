...
static void setup_shm(void) {  //共享内存进程
  ...
  shm_id = shmget(IPC_PRIVATE, MAP_SIZE, IPC_CREAT | IPC_EXCL | 0600); //该函数用来创建共享内存，可以根据此函数的返回值来访问同一共享内存  具体参数见：https://www.cnblogs.com/52php/p/5861372.html

  if (shm_id < 0) PFATAL("shmget() failed"); //如果返回值小于0，说明创建共享内存错误
